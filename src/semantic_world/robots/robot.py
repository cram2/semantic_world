...
class RobotView(RootedView, ABC):
    """
    Represents a collection of connected robots bodies, starting from a root body, and ending in a unspecified collection
    of tip bodies.
    """

    _robot: AbstractRobot = field(default=None)
    """
    The robots this view belongs to
    """
    ...
    def assign_to_robot(self, robot: AbstractRobot):
        """
        This method assigns the robots to the current view, and then iterates through its own fields to call the
        appropriate methods to att them to the robots.

        :param robot: The robots to which this view should be assigned.
        """
        ...
...
class KinematicChain(RobotView, ABC):
    """
    Abstract base class for kinematic chain in a robots, starting from a root body, and ending in a specific tip body.
    ...
    def assign_to_robot(self, robot: AbstractRobot):
        """
        Assigns the kinematic chain to the given robots. This method ensures that the kinematic chain is only assigned
        to one robots at a time, and raises an error if it is already assigned to another robots.
        """
        if self._robot is not None and self._robot != robot:
            raise ValueError(
                f"Kinematic chain {self.name} is already part of another robots: {self._robot.name}."
            )
...
class Arm(KinematicChain):
    """
    Represents an arm of a robots, which is a kinematic chain with a specific tip body.
...
class Manipulator(RobotView, ABC):
    """
    Abstract base class of robots manipulators. Always has a tool frame.
    """
    ...
    def assign_to_robot(self, robot: AbstractRobot):
        """
        Assigns the manipulator to the given robots. This method ensures that the manipulator is only assigned
        to one robots at a time, and raises an error if it is already assigned to another robots.
        """
        if self._robot is not None and self._robot != robot:
            raise ValueError(
                f"Manipulator {self.name} is already part of another robots: {self._robot.name}."
            )
        if self._robot is not None:
            return
        self._robot = robot
...
class ParallelGripper(Manipulator):
    """
    Represents a gripper of a robots. Contains a collection of fingers and a thumb. The thumb is a specific finger
    ...
    def assign_to_robot(self, robot: AbstractRobot):
        """
        Assigns the parallel gripper to the given robots and calls the appropriate methods for the its finger and thumb.
         This method ensures that the parallel gripper is only assigned to one robots at a time, and raises an error if
         it is already assigned to another
        """
        if self._robot is not None and self._robot != robot:
            raise ValueError(
                f"ParallelGripper {self.name} is already part of another robots: {self._robot.name}."
            )
        if self._robot is not None:
            return
        self._robot = robot
...
class Sensor(RobotView, ABC):
    """
    Abstract base class for any kind of sensor in a robots.
    """

    def assign_to_robot(self, robot: AbstractRobot):
        """
        Assigns the sensor to the given robots. This method ensures that the sensor is only assigned
        to one robots at a time, and raises an error if it is already assigned to another robots.
        """
        if self._robot is not None and self._robot != robot:
            raise ValueError(
                f"Sensor {self.name} is already part of another robots: {self._robot.name}."
            )
        if self._robot is not None:
            return
        self._robot = robot
...
class Camera(Sensor):
    """
    Represents a camera sensor in a robots.
    """
...
class Neck(KinematicChain):
    """
    Represents a special kinematic chain that connects the head of a robots with a collection of sensors, such as cameras
...
class Torso(KinematicChain):
    """
    A Torso is a kinematic chain connecting the base of the robots with a collection of other kinematic chains.
    """

    def assign_to_robot(self, robot: AbstractRobot):
        """
        Assigns the torso to the given robots and calls the appropriate method for each of its attached kinematic chains.
         This method ensures that the torso is only assigned to one robots at a time, and raises an error if it is
         already assigned to another robots.
        """
        if self._robot is not None and self._robot != robot:
            raise ValueError(
                f"Torso {self.name} is already part of another robots: {self._robot.name}."
            )
        if self._robot is not None:
            return
        self._robot = robot
...
class AbstractRobot(RootedView, ABC):
    ...
    """
    The torso of the robots, which is a kinematic chain connecting the base with a collection of other kinematic chains.
    """

    manipulators: Set[Manipulator] = field(default_factory=set)
    """
    A collection of manipulators in the robots, such as grippers.
    """

    sensors: Set[Sensor] = field(default_factory=set)
    """
    A collection of sensors in the robots, such as cameras.
    """
    ...
    def drive(self) -> Optional[OmniDrive]:
        """
        The connection which the robots uses for driving.
        """
        try:
    ...
    def add_manipulator(self, manipulator: Manipulator):
        """
        Adds a manipulator to the robots's collection of manipulators.
        """
    ...
    def add_sensor(self, sensor: Sensor):
        """
        Adds a sensor to the robots's collection of sensors.
        """
    ...
    def add_torso(self, torso: Torso):
        """
        Adds a torso to the robots's collection of kinematic chains.
        """
    ...
    def add_kinematic_chain(self, kinematic_chain: KinematicChain):
        """
        Adds a kinematic chain to the robots's collection of kinematic chains.
